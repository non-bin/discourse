...
task "assets:precompile:build" do
  if ENV["SKIP_EMBER_CLI_COMPILE"] != "1"
    ...
    if heap_size_limit < 2048
      STDERR.puts "Node.js heap_size_limit (#{heap_size_limit}) is less than 2048MB. Setting --max-old-space-size=600 and CHEAP_SOURCE_MAPS=1"
      compile_command =
        "JOBS=0 CI=1 NODE_OPTIONS='--max-old-space-size=600' CHEAP_SOURCE_MAPS=1 #{compile_command}"
    end
